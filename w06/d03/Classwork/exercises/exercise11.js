...
while (index < firstnamelist.length) {
	if (firstnamelist[index] == "Muffinmitts") {
		firstnamelist.splice(index, 1)
	};
	index = index + 1;
};

var scratchNSniffIndex = lastnamelist.indexOf("Scratchnsniff");

var firstieName = Math.floor(Math.random() * firstnamelist.length);
var lastieName = Math.floor(Math.random() * lastnamelist.length);

var randomName = firstnamelist[firstieName] + " " + lastnamelist[lastieName];

function randomNames(firstNameArray, lastNameArray, total_names) {
	times = 0;
	output = ""
	while (times < total_names) {
		var firstieName = Math.floor(Math.random() * lastNameArray.length);
		var lastieName = Math.floor(Math.random() * lastNameArray.length);
		output = output + firstNameArray[firstieName] + " " + lastNameArray[lastieName] + "\n";
		times = times + 1;
	};
	return output;
};
...
solution = randomNames(firstnamelist, lastnamelist, args[2]);

...
///users/page_length?page_length=5

	else if (path.split("?")[0] == '/users/page_length'){
		var numOnPage = path.split("?")[1];
		var users = [];

		for (var i = 0; i < numOnPage; i++) {
			var x = Math.floor(Math.random() * boyMusician.length);
			users.push(boyMusician[x]);
		}
		var users_in_json = JSON.stringify(users);

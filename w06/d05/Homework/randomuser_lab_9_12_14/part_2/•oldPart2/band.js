...
var server = http.createServer(function(request, response){
	...
	if (method == 'POST') && (path == '/user/create'){
		var x = math.random(0, boyMusician.length);
		var boyMusician_in_json = JSON.stringify(boyMusician[x]);
		response.end(boyMusician_in_json);

		// response = HTTParty.post("http://localhost:2000/user/create");
  //   user_id = response_hash["id"]
	} 

	else if (method == 'GET') && (path == '/user/' + user_id){

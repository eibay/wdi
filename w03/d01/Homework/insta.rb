...
p "client connected"
	request = client.gets
p "client is requesting #{request}"
	url = request.split(" ")[1]
p "client is attempting to reach #{url}"
...
p "client searched for #{searched_tag}"
		...
		ig_stuff["data"].each do |x|
			image = "<li><a href='#{x["images"]["standard_resolution"]}'><img src='#{x["images"]["low_resolution"]["url"]}'/></a></li> "
			images_html += image
		end

...
while true
  ...
  if path == "/movies"
    ...
    x += "<ol>" + titles.join('') + "</ol>"
    # client.puts x
    html = File.read('./views/imdb.html')
    new_html = html.gsub('{{titles}}', x)
    # binding.pry
    client.puts(new_html)
     elsif path == "/styles/stylesheet.css"
    client.puts File.read('./styles/stylesheet.css')
    ...
    html2 = File.read('./views/movie.html')
    movName = movie[:name]
    ...
    new_html2 = html2.gsub('{{html}}', html).gsub('{{name}}', movName).length

    #new_html3 = html2.gsub('{{name}}', movName)
    client.puts(new_html2)
  end

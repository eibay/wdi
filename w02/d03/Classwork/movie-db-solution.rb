...
while true
  ...
  if path == "/movies"
   our_html = File.read("./views/index.html")
   our_html = our_html.gsub('{{title}}', titles.join(''))
  client.puts our_html
    ...
    while i < movie_db.length
      if movie_db[i][:name] == title
        movie = movie_db[i]
    new_html = File.read("./views/index1.html")
    new_html = new_html.gsub('{{title}}',movie[:name])
    new_html = new_html.gsub('{{year}}',movie[:year])
    new_html = new_html.gsub('{{rank}}',movie[:rank])
    new_html = new_html.gsub('{{link}}',movie[:link])
      end
      i += 1
    end

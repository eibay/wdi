...
i = 0
while i < rose_git.length
  person = rose_git[i]
  gitnames.push("<li><a href=#{rose_git[i][:html_url]}'>#{rose_git[i][:login]}</a></li>")
  i += 1
end
...
while true
  ...
  if path == "/"
    html = "<h1>The Best WDI Class in History</h1>"
    html += "<ol>" + gitnames.join('') + "</ol>"
    client.puts(html)
  end
  ...
  while i < rose_git.length
    if path == "/" + rose_git[i][:login]
      html = File.read('./views/template.html')
      html = html.gsub('{{github_name}}', rose_git[index][:login])
      html = html.gsub('{{githublink}}', rose_git[index][:html_url)
      html = html.gsub('{{date_created}}', rose_git[index][:created_at])
      html = html.gsub('{{avatar_headshot}}', rose_git[index][:avatar_url])
      html = html.gsub('{{number_of_projects}}', rose_git[index][:public_repos])
      client.puts(html)
    else
      i += 1
    end
  ...
  if path == "/styles.css"
    css = File.read("./stylesheets/my_style.css")
    client.puts(css)
  end

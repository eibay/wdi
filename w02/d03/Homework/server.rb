...
#forming an array with html strings that we will later use to push out to the browser
i = 0
while i < github_users.length
  name = github_users[i]
  titles.push("<li><a href='/#{name}'>#{name}</a></li>")
  i += 1
end
...
while true
  ...
  #reads in the request, only the first line
  request = client.gets.chomp
  path = request.split(" ")[1]
  puts "Requesting #{path}"
  # acceptable_users= [github_users[:login]]
  if path == "/"
    html = "<h1>Our Students!</h1>"
    html += "<ol>" + titles.join('') + "</ol>"
    #server pushing html we just formed to the client, i am putsing to the client html
    client.puts(html)
   ...
   i= 0
   while i < github_users.length
   		if users[i]["login"] == path.split("/")[1]
   		name = users[i]["login"]
   		url = users[i]["html_url"]
   		created = users[i]["created_at"]
...
html = File.read('./views/githubprof.html')
    html = html.gsub('{{name}}', name[:login])
    html = html.gsub('{{url}}', url[:html_url])
    html = html.gsub('{{created}}', created[:created_at])
    html = html.gsub('{{avatar}}', avatar[:avatar_url])
    html = html.gsub('{{public}}', public_repos[:public_repos])
   

client.puts(html)
#   # adding a style sheet
#   # elsif path == "/styles.css"
#   #   css = File.read("./stylesheets/my_styles.css")
#   #   client.puts(css)
  end

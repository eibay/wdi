...
style = File.read('./stylesheets/style.css')
...
while i < students.length
	students_list << "<li><a href='/#{students[i]['login']}'>#{students[i]['login']}</a></li>"
	i += 1
end
...
loop do
  ...
  dirs = path.split("/")
    ...
    html = html.gsub("{{students}}", students_list.join("\n"))
    client.puts html
    puts "Sent Client index.html"
  elsif dirs.length == 2 && !dirs[1].include?("favicon")
    html = File.read("./views/student.html")
    i = 0
    while i < students.length
      if dirs[1] == students[i]["login"]
        parcel = students[i]
      end
      i += 1
    end
    i = 0
    while i < parcel.keys.length
      html = html.gsub("{{#{parcel.keys[i]}}}", parcel[parcel.keys[i]].to_s)
      i += 1
    end
    html = html.gsub("{{style}}", style)
    client.puts html
    puts "Sent Client /#{dirs[1]}"
  else
    client.puts "Doan know bout that"
  end

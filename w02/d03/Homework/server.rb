...
while n < student_db.length
	student = student_db[n]
	html_logins = File.read("./views/logins.html")
	logins_lis.push(html_logins.gsub('{{logins}}', student["login"]))
	n += 1
end
...
while true
	...
	css_path = "/style"
	html_student_page = File.read("./views/student_page.html")


	if path == "/"
		html_index = File.read("./views/index.html")
		client.puts html_index.gsub('{{logins}}', logins_lis.join('')).gsub("{{stylesheet}}", css_path)
		...
		x = 0
			while x < student_db.length
				if student_db[x]["login"] == name
					stud = student_db[x]		
				end
				x += 1
			end
			
			html = html_student_page.gsub('{{login}}', stud["login"])
			html = html.gsub('{{url}}', stud["html_url"])
			html = html.gsub('{{avatar}}', stud["avatar_url"])
			html = html.gsub('{{date}}', stud["created_at"])
			html = html.gsub('{{repos}}', stud["public_repos"].to_s)
			html = html.gsub("{{stylesheet}}", css_path)
			client.puts(html)

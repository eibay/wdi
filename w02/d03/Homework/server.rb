...
while i < students.length
	name = students[i]["login"]
	names.push("<li><a href='/rosencrantz/#{name}'>#{name}</a></li>")
	i += 1
end
...
while true
	...
	# binding.pry
	if path == "/rosencrantz"
		each_name = names.join('')
		html = File.read('./views/index.html')
		puts "sent index.html"
		html = html.gsub("{{name}}", each_name)
		client.puts html
	...
	elsif path.split("/")[1] == "rosencrantz" && path.split("/").length == 3
		i = 0
		while i < students.length
			if students[i]["login"] == path.split("/")[2]
				the_name = students[i]["login"]
				the_html_url = students[i]["html_url"]
				the_created_at = students[i]["created_at"]
				the_avatar_url = students[i]["avatar_url"]
				the_public_repos = students[i]["public_repos"]
			end
			i += 1
		end
		html = File.read('./views/student.html')
		html = html.gsub("{{the_name}}", the_name)
		html = html.gsub("{{the_html_url}}", the_html_url)
		html = html.gsub("{{the_created_at}}", the_created_at)
		html = html.gsub("{{the_avatar_url}}", the_avatar_url)
		html = html.gsub("{{the_public_repos}}", the_public_repos.to_s)

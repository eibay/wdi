...
nameArr = []
nameArrLoop = 0
while nameArrLoop < students.length
	nameArr << students[nameArrLoop]["login"]
	nameArrLoop += 1
end
...
loginArr = []
loginArrLoop = 0
while loginArrLoop < students.length
	loginArr << "<li><a href='/#{students[loginArrLoop]["login"]}'>#{students[loginArrLoop]["login"]}</a></li>"
	loginArrLoop += 1
end
...
if path == "/"
	...
	html2 = "<div><h1>Github Manifest</h1></div>"
	html2 += "<div><ul>" + loginArr.join('') + "</ul></div>"
	html = html.gsub('{{logins}}', html2)
	client.puts html
elsif path == "/styles/stylesheet.css"
	css = File.read("./styles/stylesheet.css")
	
	client.puts css
elsif path.split("/").length == 2 && path.split(".").length < 2
	name = path.split("/")[1]
	html = File.read('./views/users.html')
	html2 = "<div>
	<h1>All about #{name}</h1></div></br>"
	
	#how many months ago?

	created = students[nameArr.index(name)]["created_at"]
	createdArr = []
	createdArr = created.split("-")
	createdArr.delete_at(2)
	
	createdYear = createdArr[0].to_i
	createdMonth = createdArr[1].to_i
	t = Time.new
	tArr = []
	tArr = t.to_s.split("-")
	while tArr.length > 2
	...
	tYear = tArr[0].to_i
	tMonth = tArr[1].to_i
	yearDif = tYear - createdYear
	monthDif = tMonth - createdMonth
	totalDif = (yearDif * 12) + monthDif
	
	url = students[nameArr.index(name)]["html_url"]
	avatar = students[nameArr.index(name)]["avatar_url"]
	repos = students[nameArr.index(name)]["public_repos"]
	repos = repos.to_s
...
avatar2
	...
	html2 +="<div><p>#{name} has ruled github for #{totalDif} months!</p>"
	html2 +="<p><a href='#{url}'>click here</a> for more info, as well as to check out a whopping #{repos} public repositories!</p></div>"	
	html = html.gsub('{{time}}', html2)



	client.puts html
elsif path == "/styles/stylesheet2.css"
	css2 = File.read("./styles/stylesheet2.css")
	avatar = students[nameArr.index(name)]["avatar_url"]
	css2 = css2.gsub('{{avatar}}', avatar).gsub('{{avatar2}}', avatar2)
	client.puts css2
end
# "login"=>"gemsandjava",
#     "html_url"=>"https://github.com/gemsandjava",

...
index = 0
  subs = [
    ['{{login}}', students[index]['login']],
    ['{{avatar}}', students[index]['avatar_url']],
    ['{{link}}', students[index]['html_url']],
    ['{{created_at}}', students[index]['created_at']],
    ['{{repos}}', students[index]['public_repos']]
  ]

  login_index = 0
    if path == "/"
        while login_index < students.length

          unless login_index == 0
            html = File.read('./views/login.html')
            html = html.gsub('{{login}}', students[login_index]['login'])
            client.puts(html)
          else
...
student_page = []
while page_index < students.length
  student_page.push(students[page_index]['login'])
  page_index += 1
end

pupil = path.split('/')
if student_page[student_page]['login'].includes?(pupil[1])
# if path == "/#{students[page_index]['login']}"
# binding.pry

html = File.read('./views/students.html')
html = html.gsub('{{login}}', students[page_index]['login'] )
html = html.gsub('{{avatar}}', students[page_index]['avatar_url'])
html = html.gsub('{{link}}', students[page_index]['html_url'])
html = html.gsub('{{created_at}}', students[page_index]['created_at'] )
html = html.gsub('{{repos}}', students[page_index]['public_repos'] )
...
page_index += 1
  ...
  #     j = 0
  #     if j < subs.length
  #     html = html.gsub(subs[j][0], subs[j][1])
  #     j += 1
  #     # binding.pry
  #     index += 1
  #     client.puts(html)

...
i = 0
while i < wdi_students.length
	student = wdi_students[i]
	names.push("<li><a href='/studentswdi/#{URI.encode(student[:login])}'>#{student[:login]}</a></li>")
	i += 1
end
...
while true
	...
	if path == "/studentswdi"
		html = "<h1>GITHUB WDI</h1>"
		html += "<ol>" + names.join('') + "</ol>"
		client.puts(html)
	elsif path.split("/")[1] == "studentswdi" && path.split("/").length == 3
		name2 = URI.decode(path.split('/')[2])

	i = 0
	while i < wdi_students.length
		if wdi_students[i][:login] == name2
			student = wdi_students[i]
		end
  ...
  html = File.read('./students_github.html')

	html = html.gsub('{{loginname}}', student[:login])
	html = html.gsub('{{publicrep}}', student[:public_repos])
	html = html.gsub('{{html_url}}', student[:html_url])

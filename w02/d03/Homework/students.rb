...
while i < students.length
	student = students[i]
	logins.push("<li><a href='/#{student['login']}'>#{student['login']}</li>")
	i += 1
end
...
while true
	...
	student = path.split("/")[1]
	student_index = students.find {|i| i["login"] == student}
	replace = {"{{style}}" => "<link rel='stylesheet' type='text/css' href='../stylesheets/style.css'>",
			"{{list}}" => logins.join('')
			}

	if path == "/"
		html = File.read("./views/index.html")
		replace.each {|k, v| html = html.gsub(k, v)} # multiple gsub
		# html = html.gsub("{{style}}", "<link rel='stylesheet' type='text/css' href='../stylesheets/style.css'>").gsub("{{list}}", logins.join(''))
		client.puts html
		...
		client.puts "No favicon set"
		puts "Sent favicon error"
	elsif path.split("/")[1] == student_index["login"] # && path != "/favicon.ico" 
		html = File.read("./views/students.html")
		replacer = { "{{style}}" => "<link rel='stylesheet' type='text/css' href='../stylesheets/style.css'>",
			"{{login}}" => student_index["login"],
			"{{avatar}}" => student_index["avatar_url"],
			"{{html_url}}" => "<a href='#{student_index['html_url']}'>#{student_index['html_url']}</a>",
			"{{created_at}}" => student_index["created_at"],
			"{{public_repos}}" => student_index["public_repos"].to_s }
		replacer.each {|k, v| html = html.gsub(k, v)} # multiple gsub
		# html = html.gsub("{{style}}", "<link rel='stylesheet' type='text/css' href='../stylesheets/style.css'>").gsub("{{login}}", student_index["login"]).gsub("{{avatar}}", student_index["avatar_url"]).gsub("{{html_url}}", "<a href='#{student_index['html_url']}'>#{student_index['html_url']}</a>").gsub("{{created_at}}", student_index["created_at"]).gsub("{{public_repos}}", student_index["public_repos"].to_s) 
		client.puts html
		puts "Sent students.html"
	else

...
while x < student_db.length
	student = student_db[x]
	name.push("<li><a href='/students/#{URI.encode(student[:login])}'>#{student[:login]}</a></li>")
		x+=1
end
...
while true 
	...
	puts "Client requests #{path}."

	if path == "/rosen-students"
		main_html = File.read("./views/main.html")
		client.puts main_html.gsub("{{name}}", name.join(''))

	elsif path.split("/")[1] == "students" && path.split("/").length == 3
		student_name = path.split("/")[2]

		y = 0
		while y < student_db.length
			if student_db[y][:login] == student_name
				student_chosen = student_db[y]
			end

			y+=1
		end

	student_html = File.read('./views/student_pages.html')
	student_html = student_html.gsub('{{student_chosen[:login]}}', student_chosen[:login])
	student_html = student_html.gsub('{{student_chosen[:avatar_url]}}', student_chosen[:avatar_url])
	student_html = student_html.gsub('{{student_chosen[:html_url]}}', student_chosen[:html_url])
	student_html = student_html.gsub('{{student_chosen[:created_at]}}', student_chosen[:created_at])
	student_html = student_html.gsub('{{student_chosen[:public_repos]}}', student_chosen[:public_repos].to_s)

	client.puts student_html

	elsif path == "/main_style"
		css = File.read('./stylesheets/main_style.css')
		client.puts css

	elsif path == "/student_style"

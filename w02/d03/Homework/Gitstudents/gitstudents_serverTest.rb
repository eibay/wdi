...
if path == '/'
  ...
  binding.pry
    html = html.gsub('{{login}}', students[0]["login"])
    html = html.gsub('{{html_url}}', students[0]["html_url"])
    html = html.gsub('{{creation}}', students[0]["created_at"])
    html = html.gsub('{{avatar}}', students[0]["avatar_url"])
    html = html.gsub('{{repos}}', students[0]["public_repos"])
end
    client.puts(html)

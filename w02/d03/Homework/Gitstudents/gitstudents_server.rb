# -- Create list of logins, an array of links
# -- Display array of links so user can click on one to change path
# (join to display as text string)
# -- Test name in path for match with login
# -- Read HTML and gsub values from big array
# -- Display HTML with those values


# Variables
# students    -- big array
# login_links -- array of links
# student     -- html that gets gsubbed
...
# build array of login links
  login_links = []
  i = 0
  while i < students.length
    # a link with text as students[i]["login"] as text 
    # and students[i][html_url] as link
    login_links.push("<li><a href='/views/#{students[i][:login]}'>#{students[i][:login]}</a></li>")
    i += 1
  end
...
person = ''
  ...
  #  # find login in big array
  #   while x < students.length
  #     if person == students[i]["login"]
  #     student = login[i]
  #    end
  #         x += 1
 ...
 student = File.read('./views/students.html')
 student = student.gsub('{{login}}', :login)

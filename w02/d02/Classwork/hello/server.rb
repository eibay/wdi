#{people[name][:location]} and whose favorite flavor of ice cream is #{people[name][:flavor]}
...
while true
  ...
  if path == "/hello"
    ...
    if people.keys.include?(name)
      our_output =  "<h1>Hello, #{name.capitalize}</h1><p>You live in #{people[name][:location]} and your favorite ice cream is #{people[name][:flavor]}"

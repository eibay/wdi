 ...
 # while i >= 0
 # 	puts "#{nums[i]} bottles of beer on the wall, #{nums[i]} bottles of beer.
	# Take one down and pass it around, #{nums[i]-1} bottles of beer on the wall."
 # 	i -= 1
 # end
...
students.each { |student|
	name = student["login"]
	place = student["created_at"]
	puts "#{name} was created at #{place}"
}

...
if path = '/Inception' 
#this is the path of someone coming TO us, not us going to outside website
#if they're looking for Inception, this is what our result page might have

# goal is to get the info from an outside website 
OMDB_api = TCPSocket.new 'www.omdbapi.com', 80 
#port 80 = commonly used port for websites to access other websites
OMDB_api puts 'GET /?t = Inception'


puts "#{Time.now} - Client #{client_ip} connecting to OMDB"

json_response = omdb.api.gets
parsed_respnse = JSON.parse(json_response)


#old when we know the IP and the port

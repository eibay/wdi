...
while x < name_email.length 
	name = name_email[x][0]  # "jeff konowitch" 					= name_email[0][0]
	email = name_email[x][1] 	# "jeffkonowitch@generalassemb.ly" 		= name[0][1]

	name_split = name.split(" ") # ["jeff", "konowtich"]		= "jeff konowitch".split(" ")

	email_split_at = email.split("@") # ["jeffkonowitch", "generalassemb.ly"]	= "jeffkonowitch@generalassemb.ly".split("@")

	email_split_at_dot = email_split_at[0].split(".") # ["neel", "patel"] = ["neel.patel", "generalassemb.ly"][0].split(".")
	initial_last = name_split[0].initial.to_s << name_split.to_sym	# j 

...
def totalstops(firststop, laststop)
	return firststop - laststop
end

def totalstopsTransfer(firststop, laststop, tounion, fromunion)
	first_leg = tounion - firststop
	second_leg = fromunion - laststop
	return totalstops = first_leg + second_leg
end
binding.pry
if begline == endline
	totalstops = totalstops(subway[begline.to_sym].index(begstop).abs, subway[endline.to_sym].index(endstop).abs)
else
	totalstops = totalstopsTransfer(subway[begline.to_sym].index(begstop).abs, subway[endline.to_sym].index(endstop).abs, subway[begline.to_sym].index("Union_sq").abs, subway[endline.to_sym].index("Union_sq").abs)
end

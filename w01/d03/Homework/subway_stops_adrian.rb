...
def stops(ln, beg_st, en_st, sub)
	binding.pry
	return (sub[ln.to_sym].index(beg_st) - sub[ln.to_sym].index(en_st)).abs
end


if begline == endline 

totalstops = stops(begline, begstop, endstop, subway)
...
first_leg = stops(begline, begstop, "Union_sq", subway)
...
second_leg = stops(endline, endstop, "Union_sq", subway)

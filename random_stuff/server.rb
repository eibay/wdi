...
# url = "ws/2/artist/?query=artist:#{artist}&fmt=json"

def parse_url(url)
  path = url.split("?")[0]  /artist?specific_artist=hello
...
while true
  ...
  ######################################################################
    ...
    response["artist"].each do |artist|
      the_artist = File.read("./views/individual_artist.html")
      the_artist = the_artist.gsub("{{name}}", artist["name"])
      the_artist = the_artist.gsub("{{id}}", artist["id"])
      the_artist = the_artist.gsub("{{country}}", artist["country"].to_s)
